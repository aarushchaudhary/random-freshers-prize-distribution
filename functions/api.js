...
router.post('/login', async (req, res) => { try { const { sapId, password } = req.body; const user = await User.findOne({ sapId }); if (!user) return res.status(404).json({ success: false, message: 'User not found.' }); const isMatch = await bcrypt.compare(password, user.password); if (!isMatch) return res.status(401).json({ success: false, message: 'Invalid credentials.' }); if (user.role === 'student' && user.isEliminated) return res.status(403).json({ success: false, message: 'You have been eliminated.' }); res.json({ success: true, user }); } catch (error) { res.status(500).json({ success: false, message: 'Server error during login.' }); } });
...
io.on('connection', (socket) => {
    ...
    socket.on('admin:eliminateShape', async (data) => {
        ...
        if (shapeQuestState.target === 'boys') {
            filter.isGirl = false;
        } else if (shapeQuestState.target === 'girls') {
            filter.isGirl = true;
        }
        ...
        if (eliminatedNumbers.length > 0) {
            await User.updateMany({ assignedNumber: { $in: eliminatedNumbers } }, { isEliminated: true });
    ...
    socket.on('admin:eliminateByNumber', async (data) => { const numbers = data.numbers.map(n => parseInt(n)).filter(n => !isNaN(n)); await User.updateMany({ assignedNumber: { $in: numbers } }, { isEliminated: true }); io.emit('event:playersEliminated', { numbers }); });
    socket.on('admin:unEliminatePlayer', async (data) => { const { sapId } = data; if (!sapId) return; try { const updatedUser = await User.findOneAndUpdate( { sapId }, { isEliminated: false }, { new: true } ); if (updatedUser) { io.emit('event:playerUnEliminated', { sapId: updatedUser.sapId }); } } catch (error) { console.error('Error un-eliminating player:', error); } });
